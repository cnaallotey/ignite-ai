...
const audience = [
  ...
  "Professionals",
  "Business Owners",
  "Anyone looking to step into advanced AI roles.",
];
</script>
<template>
  <div class="w-full">
      ...
      <div class="max-w-5xl mx-auto w-full border-x border-gray-800">
        <div class="grid w-full md:grid-cols-2">
          <div class="w-full h-full flex flex-col justify-center p-4 md:p-10">
            ...
            <p class="text-white">Our courses are open to all:</p>
            <div
              v-for="item in audience"
              :key="item"
              class="border border-slate-800 min-h-20 p-4 md:text-left flex items-center md:justify-start mt-2"
            >
              <h3 class="font-medium text-lg text-white">{{ item }}</h3>
            </div>
          </div>

          <div
            class="w-full h-[500px] border-l border-slate-800 overflow-hidden flex flex-col gap-4 md:gap-6"
          >
          <img src="/images/gen-ai.png" alt="gen-ai">
          </div>
        </div>
      </div>
    </div>
